...
const roleKeywords: Record<MemberRole, string[]> = {
  ...
  VOICE_ACTOR: ["목소리", "성우", "보이스", "더빙", "내레이션", "연기", "오디오", "대사", "낮은 톤"],
  SOUND_DIRECTOR: ["bgm", "사운드", "음악", "효과음", "믹싱", "앰비언트", "asmr", "입체음향", "폴리"],
...
const topicKeywords: Record<string, string[]> = {
  ...
  일상: ["일상", "성장", "학교", "카페", "생활", "잔잔", "공감"],
  힐링: ["힐링", "따뜻", "감성", "위로", "잔잔", "asmr", "휴식"],
  BL: ["bl", "비엘", "청춘", "관계성", "팬덤"],
...
function inferTone(projectDescription: string, topics: string[]) {
  ...
  if (source.includes("어두") || source.includes("비") || source.includes("밤") || topics.includes("미스터리")) {
  ...
  if (topics.includes("힐링") || topics.includes("일상")) {
    return "편안하고 오래 머무르는 감성";
  }
  if (topics.includes("판타지")) {
    return "세계관 확장에 강한 판타지 무드";
  }
  return "장르 확장이 쉬운 멀티 콘텐츠 무드";
}

function buildProjectBrief(input: AiMatchingInput) {
  const topics = unique([...detectTopics([input.projectDescription, ...(input.genres ?? [])].join(" ")), ...(input.genres ?? [])]).slice(0, 6);
  const neededRoles = detectRoleNeeds(input.projectDescription, input.preferredRoles ?? []).slice(0, 5);
  const tone = inferTone(input.projectDescription, topics);
  const recommendedTeam = neededRoles.length > 0 ? neededRoles.map((role) => roleLabels[role]) : ["그림", "목소리", "BGM"];

  return {
    summary: `${tone}을 중심으로 ${recommendedTeam.join(", ")} 협업을 붙이면 완성도가 올라갈 프로젝트로 보여요.`,
    tone,
    topics,
    neededRoles: recommendedTeam,
  };
}
...
function buildReasonDetails(input: {
  ...
  if (input.matchedKeywords.length > 0) {
    details.push(`${input.matchedKeywords.slice(0, 4).join(", ")} 키워드가 프로필/포트폴리오와 겹칩니다.`);
  }
  ...
  if (input.featured) {
    details.push("플랫폼 추천 프로필이라 초기 협업 안정성이 높게 평가됐습니다.");
  }
...
function buildPrimaryReason(role: MemberRole, matches: string[], preferredRoleMatched: boolean) {
  ...
  if (topKeywords.length >= 2 && preferredRoleMatched) {
    return `${roleText} 포지션으로 딱 맞고, ${topKeywords.join(", ")} 감성이 포트폴리오에서 확인됩니다.`;
  }

  if (topKeywords.length > 0) {
    return `${topKeywords.join(", ")} 분위기의 작업 이력이 보여서 이번 프로젝트와 결이 잘 맞습니다.`;
  }
...
function buildSuggestedMessage(projectDescription: string, creatorName: string, role: MemberRole, matchedKeywords: string[]) {
  ...
  return `${creatorName}님, ${compactDescription} 프로젝트를 준비 중입니다. ${keywordText} 방향이 포트폴리오와 잘 맞아 보여 ${roleLabels[role]} 포지션으로 협업 제안을 드리고 싶어요.`;
}
...
function buildAssistantMessage(input: AiMatchingInput, recommendations: CreatorRecommendation[]) {
  ...
  if (!top) {
    return `말해준 내용을 보면 ${brief.summary} 다만 지금 공개된 매칭 프로필이 부족해서 추천 후보를 확정하지 못했어요. 먼저 창작자 프로필을 조금 더 모으는 게 좋아요.`;
  }

  const secondLine =
    recommendations.length > 1
      ? `2순위와 3순위는 보조 포지션으로 같이 붙이면 팀 밸런스가 좋아질 후보로 골랐습니다.`
      : `현재 데이터에서는 이 후보가 가장 강하게 맞습니다.`;

  return `작품 설명을 읽어보니 ${brief.summary} 그래서 1순위는 ${top.creator.displayName}님을 추천해요. ${top.reason} ${secondLine}`;
}

function buildFollowUpSuggestions(brief: ReturnType<typeof buildProjectBrief>) {
  const roleText = brief.neededRoles.length > 0 ? brief.neededRoles.join(", ") : "그림, 목소리, BGM";

  return [
    `${roleText} 중에서 제일 먼저 섭외할 사람을 골라줘`,
    "추천 후보에게 보낼 첫 메시지를 써줘",
    "30:30:40 기준으로 지분 제안 문구를 만들어줘",
  ];
}
